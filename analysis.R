...
l <- read_rds("data/all-jrst-data.rds")

is_len_0 <- function(x) {
  ifelse(length(x) == 0, TRUE, FALSE)
}

abstracts <- map(l, ~ .[["abstract"]]) %>% modify_if(is_len_0, ~ NA)
dates <- map(l, ~ .[["date"]])%>% modify_if(is_len_0, ~ NA)
titles <- map(l, ~ .[["title"]])  %>% modify_if(is_len_0, ~ NA)
keywords <- map(l, ~ .[["keywords"]])%>% modify_if(is_len_0, ~ NA)
...
dod <- do %>% filter(!is.na(abstract))

write_csv(dod, "data/all-jrst-data.csv")
