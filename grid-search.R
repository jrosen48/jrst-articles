...
gs <- list(min_termfreq = seq(0, 1, by = .05),
           ...
           topics_numbers = c(5, 10, 15, 20, 25, 30, 35, 40, 45, 50)) %>% 
...
mod <- function(dod, min_termfreq, max_docfreq, topics_number) {
  ...
  print(str_c("min_termfreq = ", min_termfreq, "; maxdocfreq = ", max_docfreq, "; topic number = ", topics_number))
  saveRDS(out, str_c("data/lda-out/lda-model-", min_termfreq, "-", max_docfreq, "-", topics_number, ".rds"))
  out
}
